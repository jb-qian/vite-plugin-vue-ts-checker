...
import * as vueTs from '../vue-ts-checker-npm-modules/node_modules/@volar/vue-typescript';
...
export function createProgramProxy(
	...
	if (!program) {
		...
		const vueLsHost = new Proxy({
			...
			isTsc: true,
		} as vueTs.LanguageServiceHost, {
			get: (target, property) => {
				if (property in target) {
					return target[property as keyof vueTs.LanguageServiceHost];
				}
		...
		function getVueCompilerOptions(): vueTs.VueCompilerOptions {
